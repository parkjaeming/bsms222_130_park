...
which of course change depending on the values of a,b, and c. One advantage of programming languages is that we can defin variables and write expressions with these variables, similar to how we do so in math, but obtain a numeric solution. We will wirte out general code for the equation below, but if we are asked to solve **x^2+x-1=0**, then we define:
```
a <- 1
b <- 1
c <- -1
```
which stores the values for later use. We use `<-` to assign values to the varialbes.
We can also assign values using `=` instead of `<-`. but we recommend agains using `=` to avoid confusion.
